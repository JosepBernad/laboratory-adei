...
#La variable més relacionada amb el nostre target és pdays, notem que hi ha certa relació entre els dies que passen des de l'ultim contacte i la durada de la trucada. En sentit negatiu tanbé veiem una forta relació amb el nombre de vegades que s'ha contactat durant la darrera campanya.
...
A partir d'aquest anàlisi, podem veure que les variables més relacionades són 

####Construcció del model

Per poder utilitzar el train i el test, separarem les dades aleatòriament en dos subgrups, amb unes proporcions del 75% i del 25%.
```{r}
set.seed(07111995)

id <- sample_frac(data.frame(row(dfi)),0.75)
...
m1 <- glm(y~nr.employed+euribor3m+emp.var.rate+previous+cons.price.idx+cons.conf.idx+campaign, family = binomial, data = train)
...
```{r}
m2 <- glm(y~nr.employed+emp.var.rate+previous+cons.price.idx+cons.conf.idx+campaign, family = binomial, data = train)
...
```{r}
m3 <- glm(y~nr.employed+previous+cons.price.idx+cons.conf.idx+campaign, family = binomial, data = train)
