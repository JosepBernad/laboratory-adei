...
### Age
```{r}
IQR <- calcQ(df$age)$q3 - calcQ(df$age)$q1
boxplot(df$age)
mout <- c(calcQ(df$age)$q1 - IQR*1.5, calcQ(df$age)$q3 + IQR*1.5);mout;
sout <- c(calcQ(df$age)$q1 - IQR*3, calcQ(df$age)$q3 + IQR*3);sout;
abline(h= mout, col="blue",lwd=2)
abline(h=sout,col="red",lwd=2)

outliers<-which(df$age>sout[2]);outliers;
...
###duration
```{r}
IQR <- calcQ(df$duration)$q3 - calcQ(df$duration)$q1
boxplot(df$duration)
mout <- c(calcQ(df$duration)$q1 - IQR*1.5, calcQ(df$duration)$q3 + IQR*1.5);mout;
sout <- c(calcQ(df$duration)$q1 - IQR*3, calcQ(df$duration)$q3 + IQR*3);sout;
abline(h= mout, col="blue",lwd=2)
abline(h=sout,col="red",lwd=2)
outliers<-which(df$duration>sout[2]);length(outliers);
if(length(outliers)>0){
vout[outliers]<-vout[outliers]+1
...
###campaign
```{r}
IQR <- calcQ(df$campaign)$q3 - calcQ(df$campaign)$q1
boxplot(df$campaign)
mout <- c(calcQ(df$campaign)$q1 - IQR*1.5, calcQ(df$campaign)$q3 + IQR*1.5);mout;
sout <- c(calcQ(df$campaign)$q1 - IQR*3, calcQ(df$campaign)$q3 + IQR*3);sout;
abline(h= mout, col="blue",lwd=2)
abline(h=sout,col="red",lwd=2)
outliers<-which(df$campaign>sout[2]);length(outliers);
if(length(outliers)>0){
vout[outliers]<-vout[outliers]+1
nout["campaign"]<-length(outliers)}
```
###pdays NO FUNCIONA ENCARA
```{r}
IQR <- calcQ(df$pdays)$q3 - calcQ(df$pdays)$q1
boxplot(df$pdays)
mout <- c(calcQ(df$pdays)$q1 - IQR*1.5, calcQ(df$pdays)$q3 + IQR*1.5);mout;
sout <- c(calcQ(df$pdays)$q1 - IQR*3, calcQ(df$pdays)$q3 + IQR*3);sout;
abline(h= mout, col="blue",lwd=2)
abline(h=sout,col="red",lwd=2)
```
###previous
```{r}
IQR <- calcQ(df$previous)$q3 - calcQ(df$previous)$q1
boxplot(df$previous)
mout <- c(calcQ(df$previous)$q1 - IQR*1.5, calcQ(df$previous)$q3 + IQR*1.5);mout;
sout <- c(calcQ(df$previous)$q1 - IQR*3, calcQ(df$previous)$q3 + IQR*3);sout;
abline(h= mout, col="blue",lwd=2)
abline(h=sout,col="red",lwd=2)
outliers<-which(df$previous>sout[2]);length(outliers);
if(length(outliers)>0){
vout[outliers]<-vout[outliers]+1
