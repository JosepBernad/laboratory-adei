...
var_num <-names(df)[c(1, 12:14)]
length(var_num)
summary(df[,var_num])
res <- imputePCA(df[,var_num],ncp=3)
...
##Factors
```{r}
factors <-names(df)[c(28, 29, 31)];
summary(df[,factors])
resfact <- MCA(df[,factors])
nb <- estim_ncpMCA(df[factors])
resfact <- imputeMCA(df[,factors],ncp=0);
