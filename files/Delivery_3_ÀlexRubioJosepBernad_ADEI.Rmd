---
title: "Third delivery - ADEI"
author: "Alex Rubio i Josep Bernad"
date: "4 de març de 2019"
output:
  pdf_document:
    number_sections: yes
    toc: yes
...
```{r, include=FALSE}
requiredPackages <- c("mvoutlier","chemometrics","mice","missForest","missMDA","DMwR","pbkrtest","jomo","readxl","haven","sf","rgdal","missMDA","effects","FactoMineR","car","factoextra","RColorBrewer","ggplot2","dplyr","data.table", "ggmap","ggthemes","knitr","MVA", "lmtest")

# Josep
#missingPackages <- requiredPackages[!(requiredPackages %in% as.vector(installed.packages(lib.loc="/Library/Frameworks/R.framework/Versions/3.5/Resources/library")[,"Package"]))]
...
```
Cluster 1: Es troba la gent jove treballadora que han sigut prèviamnet contactats

Cluster 2: Individus que han estat prèviamnet contactats i d'edat avançada.

Cluster 3: En aquest cas és difícil veure un patró que tots els individus segueixin.

Cluster 4: En aquest cluster es troba la gent jove que treballa com a autònom.

Cluster 5: Hi trobam individus que no són d'edat avançada, contactats durant la tardor-hivern i que estàn divorciats.

Cluster 6: Aquest cluster segueix la línia del tercer, ja que no es pot veure un patró clar a simple vista.

Cluster 7: Hi podem trobar individus que no són ancians (fins els 50 anys) que no han estat previament contactats.
...
#Individus que són més propers i més lluny al centre dels diferents clusters.
clust$desc.ind
```

En la divisió de clusters per les seves variables podem trobar dos primers clusters altament relacionats amb la variable previous, persones que havien estat contactades anteriorment. En el cas del primer, també es important veure la relació que el cluster té amb els dies que han passat des de l'ultim contacte. Del segon cluster també obtenim relacions socioeconòmiques negatives i amb una important presència.

Per altra banda el cluster numero 3 unicament obté relacions en el sentit negatiu i principalment de variables socioeconòmiques. Totalment el contrari que el cluster numero 4.

Acabem amb el cluster 5. En aquest cas els individus majoritariament havien estat contactats més d'una vegada durant la campanya actual, aquests tambñe gaudeixen d'una relació positiva amb les variables socioeconòmiques.

Pel que fa als eixos resumidament podem dir que el cluster numero 1 està ben representat en la dimensió 3 i 4. El cluster 2 està ben representat en la dimensió 2,4 i 5 però molt malt repsentat en la dimensió 1. Seguidament el cluster 3 veiem que estpa mal representat en totes les dimensions existents. El 4 gaudeix de bona representació en les dues primeres dimencions. I per acabar el cluster numero 5 gaudeix de bona representació en la dimensió 4 i 1 per contra no li beneficia la dimensió 2.
...
```{r}
hist(df$duration, freq = FALSE, main = "Histograma Duration")
curve(dnorm(x,mean(df$duration),sd(df$duration)),add=TRUE,lwd=3, col = "yellow")

#En canvi amb transformacions geomètriques:

hist(log(df$duration), freq = FALSE,main = "Histograma log(duration)")
curve(dnorm(x,mean(log(df$duration)),sd(log(df$duration))),add=TRUE,lwd=3, col = "yellow")
```
Veiem que les nostres dades no següeixen una distribució normal, per tant haurem de vigilar en el calcul de la correlació.

###Numerical Variables

Primerament comprovarem quines variables numeriques estan més relacioandes amb el nostre target numeric, fet ja en apartats anteriors.
```{r}
names(df)
vars_num<-names(df)[c(1,11:14,16:20)]
condes(df[,vars_num],which(vars_num == "duration"))

#La variable més relacionada amb el nostre target és pdays, notem que hi ha certa relació entre els dies que passen des de l'ultim contacte i la durada de la trucada. En sentit negatiu també veiem una forta relació amb el nombre de vegades que s'ha contactat durant la darrera campanya.

core <- cor(df[,vars_num])
round(core[,2],dig=3)
...
###Modelling
####M1
Comencem amb un primer model on tenim totes les variables numeriques disponibles.
```{r}
m1<-lm(duration~.,data = df[,vars_num]) 
summary(m1)

#El nostre model té una explicabilitat de les dades molt baixa, concretament d'un 0,7%. Serà complicat extreure'n resultats vàlids.

vif(m1)

```
####M2
El Variance inflation factor del model 1 ens indica que existeixen variables que no ajuden a la construcció del nostre modelm concretament les variables econòmiques emp.car.rate, cons.price.idx i euribor3m. També la variable nr.employed. 
```{r}
#En un primer moment eliminarem solament les variables econòmiques anteriors.

vars_num2 <- vars_num[c(-6,-7,-9)] #vars_num1 - emp.car.rate, cons.price.idx i euribor3m

m2<-lm(duration~.,data = df[,vars_num2]) 
summary(m2)

#L'explicabilitat de les dades ha disminuit. Mala noticia.
```
####M3
```{r}
#Tornem a provar ara eliminant la variable nr.employed.
vars_num3 <- vars_num2[c(-7)]

m3<-lm(duration~.,data = df[,vars_num3]) 
summary(m3)

vif(m3) #ja no tenim dades molestes

#L'explicabilitat de les dades ha disminuit. Mirarem si el model però ha millorat o l'hem empitjorat.
```
####M3 vs. M2
```{r}
anova(m3,m2)
```
Acceptem la hipotesis nula. Els dos models son equivalents segons el test de Fisher.

####M4
A partir d'un Stepwise Algotithm intentarem crear un nou model a partir de l'anterior.
```{r}
...
```{r}
anova(m4,m3)
#Altre vegada els dos models son equivalents.

BIC(m4,m3)
```
Tot i ser models equivalents el Bayesian Information Criteria ens diu que el model 4 és millor, per tant ens quedem amb el model 4. En tot cas, farem una ultima comprovació aplicant transformacions geometriques al nostre target, recordem que no segueix una distribució normal.

```{r}
m4new<-lm(log(duration)~.,data = df[,vars_num3]) 
summary(m4new)

#L'explicabilitat de les dades ha millorat bastant.
```
####M4 vs. M4new
```{r}
anova(m4,m4new)
#No son models equivalents
BIC(m4,m4new)

marginalModelPlots(m4new)
```
Un cop vist que els models no son equivalents i realitzat altre vegada el Bayesian Information Criteria podem dir que el nostre nou model, on em aplicat transformacions geomètriques és millor que tots els anteriors. Però que la variable pdays en causa problemes, decidim eliminar-la.

####M5

```{r}
vars_num4 <- vars_num3[c(-4)] #vars_num3 - pdays
m5 <- lm(log(duration)~.,data = df[,vars_num4]) 

summary(m5)

anova(m4new,m5) #Son model equivalents
BIC(m4new,m5) #El model 5 és millor que l'anterior. Model definitiu (a falta d'afegir els factors)
...
Complementarem el nostre model amb alguna variable factor, amb la intenció de millorar-lo.

```{r}
vars_fact<-names(df)[c(22:34,38)]
vars_fact
```
#### M6(M5 + f.jobsituation)
```{r}
m6 <- lm(log(duration)~ age + campaign + previous + cons.conf.idx + f.jobsituation, data = df) 
summary(m6)

BIC(m6,m5)
```

Afegint un factor l'explicabilitat no ha aumentat. Per altra banda veiem que dels dos models m5 i m6 és millor el primer però seguirem intentant treballar amb els nostres factors. Deixem doncs el factor f.season.

####M7 (M6 + f.marital)
```{r} 
m7 <- lm(log(duration)~ age + campaign + previous + cons.conf.idx + f.jobsituation + f.marital, data = df) 
summary(m7)

BIC(m6,m7) # Empitjorem resultats, descartem el factor
```
####M8 (M6 + f.education)
```{r} 
m8 <- lm(log(duration)~ age + campaign + previous + cons.conf.idx + f.jobsituation + f.education, data = df) 
summary(m8)

BIC(m6,m8) # Empitjorem resultats, descartem el factor
```
####M9 (M6 + f.prev_contacted)
```{r} 
m9 <- lm(log(duration)~ age + campaign + previous + cons.conf.idx + f.jobsituation + f.prev_contacted, data = df) 
summary(m9)

anova(m9)

BIC(m6,m9) # Empitjorem resultats, descartem factor
```

####M10 (M6 + f.season)
```{r} 
m10 <- lm(log(duration)~ age + campaign + previous + cons.conf.idx + f.jobsituation + f.season, data = df) 
summary(m10)

anova(m10, m6) #No son models equivalents.

BIC(m6,m10) # Millorem resultats, conservem els factors f.jobsituation i f.season
```
####M11 (f.jobsituation interaction)
```{r}
m11 <- lm(log(duration)~ (age + campaign + previous + cons.conf.idx +f.season )*f.jobsituation, data = df) 
summary(m11)

BIC(m11,m10) #No ens aporta res positiu
```

####M12 (f.season interaction)
```{r}
m12 <- lm(log(duration)~ (age + campaign + previous + cons.conf.idx + f.jobsituation) * f.season, data = df) 
summary(m12)

BIC(m12,m10) #No ens aporta res positiu
```
####M13 (f.season & f.jobsituation interaction)
```{r}
m13 <- lm(log(duration)~ age + campaign + previous + cons.conf.idx + f.jobsituation * f.season, data = df) 
summary(m13)

anova(m13,m10)# Son modelsequivalents, ens quedarem amb el més nou.

BIC(m13,m10) 
```
####M14
L'anterior model no ens aporta res positiu però realitzem un test d'efectes nets per intentar solventar aquesta situació.
```{r}
m14<-step(m13, k=log(nrow(df)))
summary(m14)
Anova(m14)
anova(m14,m10) #Son models equivalents, ens quedarem amb el millor dels dos.
BIC(m14,m10) #Ara sí !!
```
###Validació del model
```{r}
...
marginalModelPlots(m14) #No notem diferencies importants entre els nostres models i les dades.
residualPlots(m14)
```
El nostre model no gaudeix de tota la homocedasticitat que ens agradaria, però les dades ens han portat a haver-lo de considerar vàlid.
#### Anàlisis dels residus
```{r}
Boxplot(rstudent(m14), id.n=10);
abline(h = -3, col = 'red', lwd = 2)
aux<-which(rstudent(m14) <= -3);length(aux);
df[aux,]
```
Veiem que tots els outliers que destaquem son de durades dins de l'interval [4,64] el més petit de tots. I de persones amb les que no s'havia contactat previament. Probablement el nostre model tendeix a donar lleus desviacions quan parlem de trucades curtes. 

###Construcció del model
```{r}
summary(m14)
```
log(duration) = 5,76 -0.065* campaign + 0.009087 * cons.conf.idx -0.1242 * f.seasonSummer -0.0842 * f.seasonAut-Win
...
En aquesta segona part de la modelització, ens ajustarem al target factor Y, que ens explica si el client ha acceptat o no.

####Preparació

Gràcies a l'anàlisi multivariant de passades entregues, eliminarem les dades considerades com outliers.
```{r}
indSpl <- c(indiv_sup.d2, indiv_sup.d3)
dfi <- df[-indSpl]
```

####Variables relacionades amb el target Y

Amb el catdes podrem veure quines variables categòriques estàn més relacionades amb la variable target Y.

```{r}
cat_desc <- catdes(dfi,21)
...
Per poder utilitzar el train i el test, separarem les dades aleatòriament en dos subgrups, amb unes proporcions del 80 i del 20%.
```{r}
set.seed(07111995)

id <- sample_frac(data.frame(row(dfi)),0.8)
n_clmns <- as.numeric(rownames(id))
train <- dfi[n_clmns,] 
test <- dfi[-n_clmns,]
...
(nrow(train) + nrow(test)) == nrow(dfi)
```

#####Model amb variables numèriques
```{r}
cat_desc$quanti.var

m1 <- glm(y~nr.employed+euribor3m+emp.var.rate+previous+cons.price.idx+cons.conf.idx+campaign, 
          family = binomial, 
          data = train)
...
vif(m1)
```
Veiem que nr.employed, euribor3m, emp.var.rate i cons.price tenen valors molt superiors a 3, per tant eliminarem la superior (euribor3m) per veure'n el resultat

```{r}
m2 <- glm(y~nr.employed+emp.var.rate+previous+cons.price.idx+cons.conf.idx+campaign, 
          family = binomial, 
          data = train)
...
vif(m2)
```
Podem veure com alguns han baixat, però seguim tenguent valors molt alts. Per tant eliminarem la major una altre vegada, emp.var.rate.

```{r}
m3 <- glm(y~nr.employed+previous+cons.price.idx+cons.conf.idx+campaign, 
          family = binomial, 
          data = train)
...
Ara ja tenim tots els valors menors que tres, podem fer el test Anova

```{r}
Anova(m3)
```

Amb els resultats del test anova, veiem que les variables menys representades són previous i campaign, per això provarem de fer transformacions geomètriques. Descartarem del model els conceptes econòmics, ja que no tenen res a veure amb l'individu.

```{r}
m4 <- glm(y~campaign+previous+nr.employed, 
          family = binomial, 
          data = train)
...
```{r}
m5 <- glm(y~campaign+poly(previous, 2)+nr.employed, 
          family = binomial, 
          data = train)
...
Una vegada aplicada la transformació, podem veure que el valor del model a previous es millora.

```{r}
m6 <- glm(y~campaign+poly(previous, 2), 
          family = binomial, 
          data = train)
...
Finalment, es farà un test de Fisher per veure la rellevància de la variable que s'ha eliminat (nr.employed).

```{r}
anova(m5, m6, test="Chisq")
```

Com es pot veure, no hi ha diferència entre els dos models, per tant eliminarem la variable ja que no ens diu res significatiu.

#####Model amb variables factors

```{r}
cat_desc$test.chi2
```

Afegim f.education com a factor al model:

```{r}
m7<- glm(y~poly(previous, 2)+campaign+f.education,
          family = binomial,
          data = train)
...
Com podem veure, el factor f.education no ens ajuda i am més, la comparació dels dos models és que el model m5 és millor.
```{r}
BIC(m7, m5)
```


Provem amb f.poutcome
```{r}
m8<- glm(y~poly(previous, 2)+campaign+f.poutcome,
          family = binomial,
          data = train)
...
BIC(m5, m8)
```
Veiem que segueix sense ajudar-nos.


Provem amb previous però factoritzada (contactat o no contactat)
```{r}
m9<- glm(y~f.prev_contacted+campaign,
          family = binomial,
          data = train)
...
BIC(m5, m9)
```
Però segueix sense millorar.

```{r}
m10<- glm(y~poly(previous, 2)+f.poutcome,
          family = binomial,
          data = train)
...
BIC(m5, m10)
```
Utilitzar la variable factor f.poutcome tampoc és útil.


```{r}
m11 <- glm(y~f.prev_contacted+f.prev_contacted+f.season+f.jobsituation+f.age+f.education,
          family = binomial,
          data = train)
vif(m11)
```

Amb la funció step, eliminarem les variables no rellevants.

```{r}
m12 <- step(m11, k=log(nrow(train)))

BIC(m5, m12)
```
Seguim veient que el model 5 és el més indicat, utilitzant només variables continues.

#####Interaccions

```{r}
mi1 <- glm(y~(poly(previous, 2))*(campaign),
          family = binomial,
          data = train)
...
BIC(m5, mi1)
```
Veiem que la interacció no ens ofereix res.

```{r}
mi2 <- glm(y~(f.prev_contacted)*(campaign),
          family = binomial,
          data = train)
...
BIC(m5, mi2)
```
Utilitzant la variable previous factoritzada tampoc millora.

```{r}
mi3 <- glm(y~(f.prev_contacted)*(f.poutcome+f.education+f.jobsituation),
          family = binomial,
          data = train)

Anova(mi3, test="LR")
```
Tampoc amb altres combinacions de factors.

#### Model final

Un cop realitzades les proves, sel·leccionam el model 5 com a bo.
```{r}
summary(m5)
```
...
Y = 41.017394 - campaign*0.071531 - poly(previous, 2):1*5.243436 + poly(previous, 2)2*4.986830 + nr.employed*-0.008383


#### Validació

```{r}
Boxplot(rstudent(m5), id.n=2)
abline(h=c(2,-2), col="yellow", lwd = 2)

outliers <- which(rstudent(m5)>=2 | rstudent(m5)<= -2)
length(outliers)
```

Ara veurem si hi ha alguna data influent que fa possible tenir molts autliers

```{r}
influent_data <- Boxplot(cooks.distance(m5), id.n = 4); influent_data

ll_inf <- which(abs(cooks.distance(m5))<2)
length(ll_inf)
```
Per tant no tenim data influent.
...
#### Predicció
```{r}
predic <- predict(m5, type="response", newdata = test)
p_numbers <- as.numeric(predic)
...
```{r}
predic.y <- factor(ifelse(p_numbers<0.4,0,1),labels=c("predit.No","predit.Yes"))
table <- table(predic.y, test$y)
table
```

Taxa d'encert:
```{r}
100*sum(diag(table))/sum(table)
```